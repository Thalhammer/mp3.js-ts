	...
	decode(stream: MP3Stream, frame: MP3Frame) {
		...
		if (header.flags & MP3FrameHeader.FLAGS.LSF_EXT) {
			...
			if (nch === 2) {
				 ...
				 */
				if (bitrate_per_channel > 192000) { throw new Error('bad bitrate/mode combination'); }
			}
		...
		for (let sb = 0; sb < bound; sb++) {
			const nbal = BITALLOC[offsets[sb]].nbal;
			for (let ch = 0; ch < nch; ch++) { allocation[ch][sb] = stream.read(nbal); }
		}
			...
			for (let sb = 0; sb < bound; sb++) {
				for (let ch = 0; ch < nch; ch++) {
					if (index = allocation[ch][sb]) {
						...
						const scale = tables.SF_TABLE[scalefactor[ch][sb][gr >> 2]];
						for (let s = 0; s < 3; s++) {
